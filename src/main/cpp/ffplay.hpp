...
public:
	...
	static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
		...
		if (ffplay::realloc_texture(tex, sdl_pix_fmt == SDL_PIXELFORMAT_UNKNOWN ? SDL_PIXELFORMAT_ARGB8888 : sdl_pix_fmt, frame->width, frame->height, sdl_blendmode, 0) < 0)
			return -1;
		switch (sdl_pix_fmt) {
	...
	// TODO see if it needs to be moved to ffplay.cpp
...
int ffplay::audio_open(VideoState* is, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate,
	struct AudioParams *audio_hw_params) {
...
void ffplay::closeAudioDevice() { SDL_CloseAudioDevice(audio_dev); }
void ffplay::pauseAudioDevice() { SDL_PauseAudioDevice(audio_dev, 0); }
void ffplay::destroyTextures() {
	if (vis_texture)
		SDL_DestroyTexture(vis_texture);
	if (vid_texture)
...
void ffplay::set_force_refresh(int refresh) { force_refresh = refresh; }

int ffplay::get_frame_timer() { return frame_timer; }
void ffplay::set_frame_timer(int newFrame_timer) { frame_timer = newFrame_timer; }

void ffplay::update_sample_display(short *samples, int samples_size) {
	int size, len;
