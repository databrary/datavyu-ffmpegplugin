...
private:
	...
	*/
	...
	static int audio_open(int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
	{
	...
	int stream_component_open(int stream_index)
	{
		...
		case AVMEDIA_TYPE_AUDIO:
			...
			if ((ret = decoder_start(&this->auddec, audio_thread)) < 0)
				goto out;
		...
		case AVMEDIA_TYPE_VIDEO:
			...
			this->viddec->decoder_init(avctx, this->pVideoq, this->continue_read_thread);
			if ((ret = decoder_start(&this->viddec, video_thread)) < 0)
				goto out;
	...
	void video_image_display()
	{
		...
		SDL_RenderCopyEx(renderer, this->vid_texture, NULL, &rect, 0, NULL, vp->flip_v ? SDL_FLIP_VERTICAL : 0);
		if (sp) {
	...
	void video_audio_display()
	{
		...
		if (show_mode == SHOW_MODE_WAVES) {
			...
			if (rdft_bits != this->rdft_bits) {
				...
				this->rdft_data = av_malloc_array(nb_freq, 4 * sizeof(*this->rdft_data));
			}
	...
	void video_refresh(double *remaining_time)
	{
		...
		display:
			/* display picture */
			if (!display_disable && this->force_refresh && show_mode == SHOW_MODE_VIDEO && pPictq->get_rindex_shown)
				this->video_display();
		}
	...
	void seek_chapter(int incr)
	{
		...
		this->stream_seek(av_rescale_q(this->ic->chapters[i]->start, this->ic->chapters[i]->time_base, AV_TIME_BASE_Q), 0, 0);
	}
...
public:
	VideoState() :
		...
		pVidclk(new Clock(pVideoq->get_serial())),
		pAudclk(new Clock(pAudioq->get_serial())),
		//TODO fix serial for the external clock
		pExtclk(new Clock(pExtclk.serial)) {
	}
...
VideoState *VideoState::stream_open(const char *filename, AVInputFormat *iformat)
{
	...
	this->read_tid = SDL_CreateThread(read_thread, "read_thread", is);
	if (!this->read_tid) {
...
// close the VideoState Stream and and Destro SDL window
