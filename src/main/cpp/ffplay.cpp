...
public:
	...
	void set_force_refresh(int refresh) { force_refresh = refresh; }

	int get_frame_timer() { return frame_timer; }
	void set_frame_timer(int newFrame_timer) { frame_timer = newFrame_timer; }
	...
	void pauseAudioDevice() { SDL_PauseAudioDevice(audio_dev, 0); }
	...
	void video_image_display(VideoState *is) {
		...
		if (!vp->uploaded) {
			if (upload_texture(&this->vid_texture, vp->frame, &img_convert_ctx) < 0)
				return;
			vp->uploaded = 1;
	...
	void video_audio_display(VideoState *is)
	{
		...
		if (is->get_show_mode() == SHOW_MODE_WAVES) {
			...
			if (realloc_texture(&this->vis_texture, SDL_PIXELFORMAT_ARGB8888, width, height, SDL_BLENDMODE_NONE, 1) < 0)
				return;
