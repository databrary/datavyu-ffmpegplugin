...
// TODO(fraudies): Convert this into a C++ class, this needs to be clearly separated from the VideoState which is
// too broad at the moment

// Note, that I've
// - removed the subtitle encoding
// - replaced SDL thread with std::thread

// TODO: Handle av_network, av_filters for audio resampling, see 'main' and 'do_exit' methods in ffplay.c
...
typedef struct VideoState {
    ...
    // This is the buffer that holds the audio data that is to be played by the java side
    uint8_t *audio_play_buf;
    unsigned int audio_play_buf_size;

    //int audio_volume; Done on user side
    int muted;
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (!(is->continue_read_thread = new std::condition_variable())) {
        av_log(NULL, AV_LOG_FATAL, "Create condition failed\n");
        goto fail;
    }
    ...
    if (!is->read_tid) {
        av_log(NULL, AV_LOG_FATAL, "Create reader thread failed\n");
fail:
        stream_close(is);
        return NULL;
    }
...
static void stream_close(VideoState *is)
{
    ...
    delete id->continue_read_thread;
...
{
    // Do only if we are not paused
    if (!is->paused) {
        toggle_pause(is);
    }
}

static void stream_play(VideoState *is)
{
    // Do only if we are paused
    if (is->paused) {
        toggle_pause(is);
    }
}
...
//      of the seek_pos/seek_rel variables
        ...
        if (infinite_buffer<1 &&
            ...
            is->continue_read_thread->wait_for(locker, timeout, []{return true;});
            locker.unlock();
            continue;
        }
        if (!is->paused &&
...
display:
        ...
        if (!display_disable && is->force_refresh && is->show_mode == SHOW_MODE_VIDEO && is->pictq.rindex_shown)
            vp_show = is->pictq->frame_queue_peek_last();
            //video_display(is);
    }
    is->force_refresh = 0;
/*    if (show_status) {
