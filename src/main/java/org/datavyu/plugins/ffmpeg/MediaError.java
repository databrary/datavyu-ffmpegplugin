...
/**
 * Media error codes that are also used in the native code to communicate
 * the error through an error number to the JVM.
 *
 * The error messages are loaded from a properties file. Currently,
 * properties only exist in English.
 *
 * Note, that error codes are kept as much as possible compatible with those
 * from javafx. Thus, the numbering may appear a bit jumpy.
 */
