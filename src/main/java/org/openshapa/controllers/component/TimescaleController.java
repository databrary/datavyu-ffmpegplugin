...
public final class TimescaleController {
	...
	private void fireJumpEvent(final long jumpTime, final boolean togglePlaybackMode) {
		...
		synchronized (this) {
			TimescaleEvent e = new TimescaleEvent(this, jumpTime, togglePlaybackMode);
	...
	private class TimescaleEventListener extends MouseInputAdapter {
		...
		private boolean isMouseInsideTimescale(final MouseEvent e) {
			return e.getX() >= timescaleModel.getPaddingLeft() && (e.getX() + timescaleModel.getPaddingRight() < view.getSize().width);
		}
		...
		private long getNewNeedlePosition(final MouseEvent e) {
			final int dx = Math.min(Math.max(e.getX()
					- timescaleModel.getPaddingLeft(), 0),
			...
			double newTime = (dx + (viewableModel.getZoomWindowStart() * ratio))
					/ ratio;
			newTime = Math.min(Math.max(newTime, 0), viewableModel
					.getZoomWindowEnd());
