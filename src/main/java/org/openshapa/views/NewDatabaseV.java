...
public final class NewDatabaseV extends OpenSHAPADialog {
    ...
    private UserMetrix logger = UserMetrix.getInstance(NewDatabaseV.class);
    ...
    private void okButtonActionPerformed(final java.awt.event.ActionEvent evt) {// GEN-FIRST:event_okButtonActionPerformed
        try {
            ...
            if (!model.isValidDatabaseName(getDatabaseName())) {
                throw new LogicErrorException(r.getString("Error.invalidName"));
            }

            model.setName(getDatabaseName());
            model.setDescription(getDatabaseDescription());

            OpenSHAPA.getProjectController().setDatabase(model);
            OpenSHAPAView s =
