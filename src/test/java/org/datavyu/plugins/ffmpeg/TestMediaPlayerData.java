...
/**
 * Tests players defined in {@link PlayerType} with a list of movies in {@link
 * MediaPlayerBuilder#movieFiles} to make sure that all the players match the requirements of a
 * media player, this suite cover calls made from Java to the native player through Java Native
 * Interface and assert if the behavior of the native side is adequate to the sent command. The
 * player state {@link PlayerStateEvent.PlayerState} must be checked after each action and its value
 * should match the current state of the player, note that we have to sleep the current thread for
 * {@link #SLEEP_DURATION_IN_MILLIS} in order to wait for the native side to update its state
 *
...
public class TestMediaPlayerData {
  ...
  private void sleep(final long timeInMillis, final boolean log) {
    try {
      if (log) LOGGER.info("Sleep for " + timeInMillis + " ms");
  ...
  /**
   * Tests {@link MediaPlayer#init()} and {@link MediaPlayer#dispose()} methods that will initialize
   * and destroy resources for the media player. All commands after a dispose call must be ignored
   * and {@link MediaPlayer#getState()} should return null if the player is disposed
   */
  @Test(
  ...
  /**
   * Tests {@link MediaPlayer#play()} and {@link MediaPlayer#stop()} methods that will start and
   * stop, respectively the media player.
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
  ...
  /**
   * Tests {@link MediaPlayer#pause()} method that will toggle between pause and play. The player
   * state {@link PlayerStateEvent.PlayerState} must be set to PAUSED when the player is paused.
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
  ...
  /**
   * Tests if the New Rate is persisted correctly in the native side and tests the behavior of the
   * media player after a pause and stop actions where the rate should be set to 1x after a stop and
   * don't change after a pause Note: this test will not cover the different playback rate speeds,
   * please check {@link TestPlaybackRate} for this matter.
   *
  ...
  /**
   * Tests if {@link MediaPlayer#getFps()} method matches the pre-defined fps value in {@link
   * MediaPlayerBuilder.Movie#fps }.
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
  ...
  /**
   * Tests if {@link MediaPlayer#getImageWidth()} and {@link MediaPlayer#getImageHeight()} methods
   * match the pre-defined {@link MediaPlayerBuilder.Movie#width } and {@link
   * MediaPlayerBuilder.Movie#height}, respectively.
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
   ...
   * of seeks {@link #NUMBER_OF_SEEK} to a random timestamp and check if the the player accuracy is
   * within a threshold {@link #SEEK_TOLERANCE_IN_SECONDS}
   *
   * <p>The FFmpeg Players don't perform well while seeking, testing are showing a tolerance of 6
   * sec in order to pass all the Asserts, we will keep it as a soft assert for now, However we
   * won't tolerate more than 30% failed seek with 100 ms as threshold.
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
   ...
   *
   * <p>IMPORTANT: Only available for the MPV player, the ffmpeg SDL and Java players can step
   * forward. however, the reported time after a seek is NaN which make the test fails
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
  ...
  public void testStepForward(Movie movie, PlayerType playerType) {
    ...
    if (playerType == PlayerType.MPV) {
      Assert.assertTrue(
          Math.abs(mediaPlayer.getPresentationTime() - randomTime) < SEEK_TOLERANCE_IN_SECONDS);
    } else {
   ...
   *
   * <p>IMPORTANT: Only available for the MPV player, the ffmpeg SDL and Java players cannot step
   * backward. however, the reported time after a seek is NaN which make the test fails
   */
  @Test(
      dataProvider = "players",
      dataProviderClass = MediaPlayerBuilder.class,
