...
public final class UITimestampTest extends OpenSHAPATestClass {
     ...
     * NEEDS TO BE REWRITTEN FOR FEST AFTER BUG IS FIXED.
     *//* BugzID:540
    ...
    public void testTimestampPasting() {
        ...
        for (int i = 0; i < numOfTests; i++) {
            ...
            onset.selectAll();
            onset.pressAndReleaseKey(KeyPressInfo.keyCode(
                    KeyEvent.VK_V).modifiers(Platform.controlOrCommandMask()));

            offset.selectAll();
            offset.pressAndReleaseKey(KeyPressInfo.keyCode(
                    KeyEvent.VK_V).modifiers(Platform.controlOrCommandMask()));
     ...
     * Create new cells.
     */
    private Vector<SpreadsheetCellFixture> createNewCells(int amount) {
        String varName = "t";
